from parser.models import (
    CategoryItem, CategoryBreakdown, ExchangeStats, SessionStats,
...
def make_exchange(input_t=100, cache_read=500, cache_create=200, output=50, breakdown=None):
...
def test_project_aggregates_across_sessions():
    ...
    s1.exchanges.append(make_exchange(input_t=100, breakdown=make_breakdown(messages_tokens=100)))
    s2 = SessionStats(session_id="s2", display_name="s2abc", first_timestamp=None)
    s2.exchanges.append(make_exchange(input_t=200, breakdown=make_breakdown(skills_tokens=200)))
...
def test_global_aggregates_across_projects():
    ...
    s1 = SessionStats(session_id="s1", display_name="s1abc", first_timestamp=None)
    s1.exchanges.append(make_exchange(input_t=100, output=10, breakdown=make_breakdown(messages_tokens=100)))
    p1.sessions = [s1]

    p2 = ProjectStats(project_slug="p2", display_name="p2")
    s2 = SessionStats(session_id="s2", display_name="s2abc", first_timestamp=None)
    s2.exchanges.append(make_exchange(input_t=300, output=30, breakdown=make_breakdown(skills_tokens=300)))
    p2.sessions = [s2]
